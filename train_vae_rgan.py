...
params['load_weights'] = (140, 1900) # version/epoch tupple pair None
...
params['disc_seq_length'] = 4
params['lambda_adv'] = 0.5
params['lambda_recon'] = 50
params['comments'] = 'first try with ae_rgan v2'
...
lstm_input = enc_output[:, slice(0,params['lstm_seq_length']), :] # (b_s, lstm_seq_length, z_dim)
lstm_preds = []
...
for i in range(params['seq_length']/params['skip_frames']-params['lstm_seq_length']):
    ...
    lstm_input = lstm_input[:, slice(i, i + params['lstm_seq_length']), :]
...
recon_loss = lasagne.objectives.squared_error(critic_input_recon, images[:, slice(0,params['disc_seq_length']), :, :, :]).mean()
...
# mse_enc_loss = lasagne.objectives.squared_error(lstm_preds, enc_output).mean()
...
# rec_fn_output['grads_norm'] = rec_grads_norm
...
for epoch in range(3000000):
    gen_err, disc_err, enc_err = 0, 0, 0

...
# recurrent structure for latent space prediction. 
def recurrent(input_var=None, num_units=512, batch_size=64, seq_length=1, grad_clip=100):
    recurrent = []

    theano_rng = RandomStreams(rng.randint(2 ** 15))
    # we want noise to match tanh range of activation ([-1,1])  
    noise = theano_rng.uniform(size=(batch_size, seq_length, num_units), low=-1.0, high=1.0)
    input_var = noise if input_var is None else input_var
    
    recurrent.append(ll.InputLayer(shape=(batch_size, seq_length, num_units), input_var=input_var))
...
def encoder_adapter(seq_length=1, z_dim=512):
    reducer, expander  = [], []
    
    reducer.append(ll.InputLayer(shape=(None, seq_length, 3, 80, 160)))

    reducer.append(ll.ReshapeLayer(reducer[-1], (-1, 3, 80, 160)))

    expander.append(ll.InputLayer(shape=(None, z_dim)))

    expander.append(ll.ReshapeLayer(expander[-1], (-1, seq_length, z_dim)))

    return reducer[-1], expander[-1]
